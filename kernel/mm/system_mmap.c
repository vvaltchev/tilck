...
struct memory_region_t mem_regions[MAX_MEM_REGIONS];
...
{
   const struct memory_region_t *m1 = a;
   const struct memory_region_t *m2 = b;
...
void *system_mmap_get_ramdisk_vaddr(int ramdisk_index)
{
   ...
   for (int i = 0; i < mem_regions_count; i++) {
      ...
      struct memory_region_t *m = mem_regions + i;
...
{
   struct memory_region_t *ma = mem_regions + i;
   const int rem = mem_regions_count - i - 1;

   memcpy(ma, ma + 1, (size_t) rem * sizeof(struct memory_region_t));
...
STATIC void swap_mem_regions(int i, int j)
{
   ...
   struct memory_region_t temp = mem_regions[i];
...
STATIC void merge_adj_mem_regions(void)
{
   for (int i = 0; i < mem_regions_count - 1; i++) {
      ...
      struct memory_region_t *ma = mem_regions + i;
      struct memory_region_t *ma_next = ma + 1;
...
STATIC bool handle_region_overlap(int r1_index, int r2_index)
{
   ...
   struct memory_region_t *r1 = mem_regions + r1_index;
   struct memory_region_t *r2 = mem_regions + r2_index;
...
bool
linear_map_mem_region(struct memory_region_t *r, uptr *vbegin, uptr *vend)
{
