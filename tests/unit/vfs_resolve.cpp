...
static void reset_all_fs_refcounts()
{
   ...
   atomic_store(&fs1.ref_count, 1);
   atomic_store(&fs2.ref_count, 1);
   atomic_store(&fs3.ref_count, 1);
}
...
static void check_all_fs_refcounts()
{
   ...
   ASSERT_EQ(atomic_load(&fs1.ref_count), 5);
   ASSERT_EQ(atomic_load(&fs2.ref_count), 2);
   ASSERT_EQ(atomic_load(&fs3.ref_count), 2);
}
