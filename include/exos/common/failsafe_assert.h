 ...
 * we defined NO_EXOS_ASSERT but, at the same time, we'd like code in common/
 ...
 * use different flags (allowing the exOS's ASSERT) specifically for code in
 * common/, but not for the rest (of the bootloader in this case).
 */

#ifndef _EXOS_BASIC_DEFS_H
#error This header must be included AFTER common/basic_defs.h
#endif

#if defined(NO_EXOS_ASSERT) && !defined(ASSERT)
#define ASSERT(x)
#endif
